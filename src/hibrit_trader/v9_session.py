"""V9 senaryo motoru - v7 + TEK fark likidite tabani $300k, dokuzuncu paralel sanal yarışçı.

Diğer motorlara (v2..v8/gölge) SIFIR dokunuş. Sadece şu dosyalara yazar:
  data/v9_state.json   (sanal bakiye + açık pozisyonlar)
  data/v9_trades.jsonl (her sanal kapanışta kayıt)

V9 = V7'NİN BİREBİR KOPYASI + TEK fark (2026-07-04):
  LİKİDİTE : taban $100k yerine $300k. Başka HİÇBİR şey değişmez.
  GİRİŞ    : liq >= $300k VE 10 <= chg_h1 <= 50 (bant v7 ile birebir).
  ÇIKIŞ    : tp_2 / stop_felaket (-%10) / stop_gec (30dk sabır sonrası -%2) /
             timeout_60. sol_chg_h1 kaydı v7 ile aynı.

Fill'ler sanal: gerçek fiyat + v2'nin likidite-slippage modeli + gas.
Kadans v2 ile aynı; 1/8 interval faz kaydırmalı (gölge:1/2, v6:3/8,
v7:5/8, v4:3/4, v8:7/8, v9:1/8). V9_ENABLED=0 ile kapatılır.
"""
...
# ---- V9 eşikleri (v7 ile aynı zemin + TEK fark: liq tabanı $300k) --------------
CHG_H1_MIN = float(os.getenv("V9_CHG_H1_MIN", "10"))
CHG_H1_MAX = float(os.getenv("V9_CHG_H1_MAX", "50"))   # v7 ile aynı bant
LIQ_MIN_USD = float(os.getenv("V9_LIQ_MIN_USD", "300000"))  # TEK fark: 100k yerine 300k
MAX_SLOTS = 5
START_BALANCE = float(os.getenv("V9_START_BALANCE", "1000"))
TP_PCT = 2.0            # giriş +%2 görülünce kâr al (v7 ile aynı)
GRACE_SEC = 30 * 60     # ilk 30dk aşağıda stop yok (sabır)
LATE_STOP_PCT = -2.0    # 30dk sonrası: girişin -%2 altı SAT
DISASTER_PCT = float(os.getenv("V9_DISASTER_PCT", "-10"))  # her an mutlak taban (v7 ile aynı)
CEILING_SEC = 60 * 60   # 60dk tavan
SOL_H1_MIN = float(os.getenv("MOM_SOL_H1_MIN", "0"))   # rejim eşiği v7 ile aynı
...
class V9Engine:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            try:
                p.rename(backup)
                log.critical("v9 state bozuk, yedeğe taşındı: %s", backup)
            except OSError:
                log.critical("v9 state bozuk ve yedeklenemedi, temiz başlanıyor")
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            fh.close()
            log.critical("V9: başka bir instance çalışıyor, motor başlatılmıyor")
            return False
    ...
    def run_forever(self) -> None:
        if not self._acquire_lock():
            return
        log.warning(
            "V9 senaryo başladı (v7 + liq 300k) - sanal $%.2f · slot %d · "
    ...
    # ---- Rejim (v7 ile aynı: eşik 0, fail-open, kendi cache'i) -----------------
    ...
    # ---- Giriş (TEK fark: liq >= $300k; h1 bandı 10..50 v7 ile birebir) ---------
    def _enter(self, client: httpx.Client) -> None:
        ...
        try:
            ...
            log.debug("v9 scan hatası", exc_info=True)
            return
        ...
        for pr in pairs:
            ...
            if not (CHG_H1_MIN <= getattr(pr, "chg_h1", 0.0) <= CHG_H1_MAX):
                continue  # v7 bandı: dikey pump tepesi dışarıda
            cands.append(pr)
        ...
        try:
            ...
            log.debug("v9 rejim: sol_chg_h1 alınamadı, filtre atlandı", exc_info=True)
        if sol_h1 is not None and sol_h1 < SOL_H1_MIN:
            if not self._regime_logged:
                self._regime_logged = True
                log.warning("V9 REJIM: sol_chg_h1 %.2f%% < %.2f%%, giriş yok", sol_h1, SOL_H1_MIN)
            return
    ...
    def _open_position(self, pair, usd: float, sol_h1: float | None = None) -> bool:
        ...
        pos = {
            ...
            "sol_chg_h1": sol_h1,   # rejim analizi için kayıt (v7 ile aynı)
        ...
        self._save()
        log.warning("V9 BUY %s $%.2f @ %.8g (h1 %.1f%%, liq $%.0f)",
    ...
    def _manage_exits(self, client: httpx.Client) -> None:
        ...
        for pos in list(self.positions):
            ...
            if pnl_pct >= TP_PCT:
                ...
                reason = "stop_felaket"  # her an geçerli mutlak taban (v7 ile aynı)
    ...
    def _close_position(self, pos: dict, price: float, reason: str, now: float) -> None:
        ...
        try:
            ...
            pass
        self._save()
        log.warning("V9 SELL %s pnl $%.2f (%.2f%%) - %s, hold %.0fs (mfe %.1f%% mae %.1f%%)",
