"""V8 senaryo motoru - gölgenin birebir kopyası + 4 fark, sekizinci paralel sanal yarışçı.

Diğer motorlara (v2/v3/v4/v5/v6/v7/gölge) SIFIR dokunuş. Sadece şu dosyalara yazar:
  data/v8_state.json   (sanal bakiye + açık pozisyonlar)
  data/v8_trades.jsonl (her sanal kapanışta kayıt)

V8 = GÖLGENİN BİREBİR KOPYASI + şu farklar (2026-07-04):
  LİKİDİTE : taban $100k yerine $200k (daha da ucuz friction, daha seçici havuz).
  GİRİŞ    : chg_h1 bandı 20..50 (20 altı girmez, 50 üstü girmez).
             Sıralama en yüksek h1 önce.
  TP       : +%2 yerine +%3 kâr hedefi (tp_3).
  SÜRE     : 30dk sabır yok; MUTLAK 20dk tavan. Pozisyon +%3'ü görürse sat;
             20dk dolduğunda NEREDE olursa olsun (artı/eksi/nötr) kapat
             (timeout_20). 60dk tavan YOK, maksimum tutuş 20dk.

Korunanlar (gölgeden aynen): safety taraması, rejim filtresi (sol_h1 < 0
giriş yok), cooldown'lar (kayıp çıkış 60dk, diğeri 15dk), 5 slot, eşit
bölüşüm, aynı fill/slippage/gas modeli. sol_chg_h1 kaydı dahil.

Fill'ler sanal: gerçek fiyat + v2'nin likidite-slippage modeli + gas.
Kadans v2 ile aynı; 7/8 interval faz kaydırmalı (gölge:1/2, v6:3/8,
v7:5/8, v4:3/4, v8:7/8). V8_ENABLED=0 ile kapatılır.
"""
...
# ---- V8 eşikleri (gölge zemini + 4 fark) --------------------------------------
CHG_H1_MIN = float(os.getenv("V8_CHG_H1_MIN", "20"))    # fark: 10 yerine 20
CHG_H1_MAX = float(os.getenv("V8_CHG_H1_MAX", "50"))    # fark: üst bant eklendi
LIQ_MIN_USD = float(os.getenv("V8_LIQ_MIN_USD", "200000"))  # fark: 100k yerine 200k
MAX_SLOTS = 5
START_BALANCE = float(os.getenv("V8_START_BALANCE", "1000"))
TP_PCT = 3.0            # fark: giriş +%3 görülünce kâr al (gölge: +%2)
CEILING_SEC = 20 * 60   # fark: MUTLAK 20dk tavan (gölge: 30dk sabır + 60dk tavan)
...
STATE_FILE = "v8_state.json"
TRADES_FILE = "v8_trades.jsonl"
...
class V8Engine:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            try:
                p.rename(backup)
                log.critical("v8 state bozuk, yedeğe taşındı: %s", backup)
            except OSError:
                log.critical("v8 state bozuk ve yedeklenemedi, temiz başlanıyor")
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            fh.close()
            log.critical("V8: başka bir instance çalışıyor, motor başlatılmıyor")
            return False
    ...
    # ---- Ana döngü (v2 ile aynı kadans, 7/8 interval faz kaydırma) ------------
    def run_forever(self) -> None:
        if not self._acquire_lock():
            return
        log.warning(
            "V8 senaryo başladı (gölge + 200k/20..50/tp3/20dk) - sanal $%.2f · slot %d · "
            "giriş liq>=$%.0f + h1 %.0f..%.0f · çıkış tp+%.0f%% / mutlak tavan %dm",
            self.balance, MAX_SLOTS, LIQ_MIN_USD, CHG_H1_MIN, CHG_H1_MAX,
            TP_PCT, CEILING_SEC // 60,
        )
        self._save()
    ...
    # ---- Giriş (liq >= $200k + h1 bandı 20..50, en yüksek h1 önce) --------------
    def _enter(self, client: httpx.Client) -> None:
        ...
        try:
            ...
            log.debug("v8 scan hatası", exc_info=True)
            return
        ...
        for pr in pairs:
            ...
            if not (CHG_H1_MIN <= getattr(pr, "chg_h1", 0.0) <= CHG_H1_MAX):
                continue  # 20 altı ivmesiz, 50 üstü dikey pump tepesi: ikisi de dışarıda
            cands.append(pr)
        ...
        try:
            ...
            log.debug("v8 rejim: sol_chg_h1 alınamadı, filtre atlandı", exc_info=True)
        if sol_h1 is not None and sol_h1 < SOL_H1_MIN:
            if not self._regime_logged:
                self._regime_logged = True
                log.warning("V8 REJIM: sol_chg_h1 %.2f%% < %.2f%%, giriş yok", sol_h1, SOL_H1_MIN)
            return
    ...
    def _open_position(self, pair, usd: float, sol_h1: float | None = None) -> bool:
        ...
        pos = {
            ...
            "sol_chg_h1": sol_h1,   # rejim analizi için kayıt (gölgede eksikti)
        ...
        self._save()
        log.warning("V8 BUY %s $%.2f @ %.8g (h1 %.1f%%, liq $%.0f)",
                    pair.name, usd, eff_price, pair.chg_h1, pair.liquidity_usd)
        return True

    # ---- Çıkış: tp_3 / timeout_20 (mutlak tavan, nerede olursa olsun kapat) -----
    def _manage_exits(self, client: httpx.Client) -> None:
        ...
        for pos in list(self.positions):
            ...
            reason = None
            if pnl_pct >= TP_PCT:
                reason = "tp_3"          # giriş +%3 görüldü: kârı al
            elif age >= CEILING_SEC:
                reason = "timeout_20"    # mutlak tavan: artı/eksi/nötr farketmez
            if reason:
    ...
    def _close_position(self, pos: dict, price: float, reason: str, now: float) -> None:
        ...
        # kayıp çıkış 60dk, diğeri 15dk; v8'de stop dalı yok, ölçüt gerçekleşen pnl
        ...
        try:
            ...
            pass
        self._save()
        log.warning("V8 SELL %s pnl $%.2f (%.2f%%) - %s, hold %.0fs (mfe %.1f%% mae %.1f%%)",
