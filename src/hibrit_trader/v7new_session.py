"""V7NEW paper motoru (22 Tem 2026, kullanici talebi).

v7new klonu, tek fark: TP +%2 yerine +%5 (tp_5). Kalan kurallar ayni:
stop yok, felaket -%20 kuyruk kapagi, 60dk kosulsuz timeout,
liq>=$100k, h1 10..50, rejim esigi. SABIT PAPER, v7new_* dosyalari.
"""
...
# ---- V7NEW esikleri (kullanici kural seti 2026-07-15) --------------------
CHG_H1_MIN = float(os.getenv("V7NEW_CHG_H1_MIN", "5"))
CHG_H1_MAX = float(os.getenv("V7NEW_CHG_H1_MAX", "45"))
LIQ_MIN_USD = float(os.getenv("V7NEW_LIQ_MIN_USD", "100000"))
MAX_SLOTS = 5
START_BALANCE = float(os.getenv("V7NEW_START_BALANCE", "1000"))
TP_PCT = float(os.getenv("V7NEW_TP_PCT", "5.0"))
SOL_H1_MIN = float(os.getenv("V7NEW_SOL_H1_MIN", "0.35"))
...
TIMEOUT_MIN = float(os.getenv("V7NEW_TIMEOUT_MIN", "60"))
...
FELAKET_PCT = float(os.getenv("V7NEW_FELAKET_PCT", "-20"))
...
EXIT_SLIPPAGE_BPS = {"tp_5": 150, "stop_gec": 300, "stop_felaket": 1000,
                     "timeout_60": 300}
STOP_RETRY_ADET = 3
STOP_RETRY_SEC = 3.0
...
STATE_FILE = "v7new_state.json"
TRADES_FILE = "v7new_trades.jsonl"
...
class V7NewEngine:
    ...
    def __init__(self, settings) -> None:
        ...
        self._huni = HuniSayac("V7NEW")
        self._lock_fh = None
        self._son_exec_neden: str | None = None
        self._belirsiz_aday: dict | None = None
        # 16 Tem: CANLI_MOTOR env swap altyapisi. Default paper; CANLI_MOTOR=v7new
        # secilirse make_exec_broker (live/dryrun) devreye girer.
        self._exec, self._exec_arizali = init_motor_exec("v7new")
        self._load()
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            try:
                p.rename(backup)
                log.critical("v7new state bozuk, yedege tasindi: %s", backup)
            except OSError:
                log.critical("v7new state bozuk ve yedeklenemedi, temiz baslaniyor")
    ...
    def _restore_day_realized(self) -> None:
        ...
        try:
            ...
            log.debug("V7NEW gun ici pnl geri yuklenemedi", exc_info=True)
    ...
    def _entries_blocked(self) -> str | None:
        ...
        if kill_is_active():
            if not self._kill_logged:
                ...
                log.critical("V7NEW: kill-switch AKTIF, yeni girisler durdu (cikislar suruyor)")
            return "kill_switch"
        if self._kill_logged:
            self._kill_logged = False
            log.warning("V7NEW: kill-switch kalkti, girisler serbest")
        if DAILY_LOSS_LIMIT_USD > 0 or self._pct_limit_aktif():
            ...
            if limit is None and not kesin:
                if not self._limit_belirsiz_logged:
                    ...
                    log.critical("V7NEW: gun limiti hesaplanamadi, yeni giris kapali (fail-closed)")
                return "daily_limit_belirsiz"
            if self._limit_belirsiz_logged:
                self._limit_belirsiz_logged = False
                log.warning("V7NEW: gun limiti hesaplandi, belirsizlik kalkti")
            if limit is not None and self._day_realized <= -limit:
                if not self._limit_logged:
                    self._limit_logged = True
                    log.critical(
                        "V7NEW: gunluk zarar limiti asildi ($%.2f <= -$%.2f), "
    ...
    def _canli_mtm(self) -> float | None:
        try:
            ...
            log.debug("V7NEW canli MTM okunamadi", exc_info=True)
        return None
    ...
    def _gun_limiti(self) -> tuple[float | None, bool]:
        ...
        self._save()
        log.warning("V7NEW gun limiti sabitlendi: MTM $%.2f x %%%g = $%.2f",
    ...
    def _exec_fill(self, yon: str, token_address: str, *, usd: float = 0.0,
        ...
        try:
            ...
            log.error("V7NEW yurutme hatasi (%s %s): %s", yon, token_address[:8], e)
            fill = None
    ...
    def _acquire_lock(self) -> bool:
        import fcntl
        p = self._path("v7new_engine.lock")
        p.parent.mkdir(parents=True, exist_ok=True)
        fh = p.open("w")
        try:
            ...
            fh.close()
            log.critical("V7NEW: baska bir instance calisiyor, motor baslatilmiyor")
            return False
    ...
    def run_forever(self) -> None:
        if not self._acquire_lock():
            return
        log.warning(
            "V7NEW paper basladi (TP=+%%%.1f tek cikis, stop yok) - sanal $%.2f · "
        ...
        # v7d 7/8 kullaniyor, v7 1/1, momentum 5/8, v6 farkli. v7new faz: 6/8.
        time.sleep(SCAN_INTERVAL_SEC * 6 / 8)
        while True:
            try:
                self.tick()
            except Exception:
                log.exception("v7new tick hatasi")
            ...
            while True:
                ...
                try:
                    ...
                    log.exception("v7new hizli cikis hatasi")
    ...
    def _belirsiz_takip(self) -> None:
        ...
        if durum == "gerceklesti" and detay and detay.get("fiyat", 0) > 0:
            ...
            log.warning("V7NEW BELIRSIZ SONUC %s: tx zincirde yok, iptal", aday["pair"])
        else:
            log.critical("V7NEW BELIRSIZ SONUC %s: cozulemedi (%s)", aday["pair"], durum)
    ...
    def _belirsiz_pozisyon_ac(self, aday: dict, detay: dict) -> None:
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        log.warning("V7NEW BUY (mutabakat) %s $%.2f @ %.8g", aday["pair"], usd, entry)
    ...
    def _enter(self, client: httpx.Client) -> None:
        ...
        try:
            ...
            log.warning("V7NEW giris tick atlandi, tarama hatasi: %r", e)
            return
        ...
        for pr in pairs:
            ...
            _izin, _red_nedeni = aday_paylastir.iddia_et(pr.token_address, "v7new", pr.name)
            if not _izin:
                continue
        ...
        for pair in cands:
            ...
            try:
                ...
                safety_reject_kaydet(pair, "V7NEW", "safety_hata", type(e).__name__)
                continue
            time.sleep(0.2 if self._aggressive else 1.5)
            if not report.ok:
                safety_reject_kaydet(
                    pair, "V7NEW", report.kapi or "safety_red",
    ...
    def _open_position(self, pair, usd: float, sol_h1: float | None = None,
        ...
        if self.balance < usd + gas:
            return False
        taze = taze_teyit(pair, "V7NEW", client)
        if taze.iptal:
            log.warning("V7NEW GIRIS IPTAL %s: taze fiyat taramanin %%%.2f ustunde (kaynak %s)",
        ...
        if not devam:
            ...
            "pool_yas_dk": (round((time.time() - float(pair.pool_created_at)) / 60.0, 1)
                ...
                log.critical("V7NEW GIRIS BELIRSIZ %s: zincir mutabakati bekleniyor",
                             pair.name)
                return False
            log.error("V7NEW GIRIS IPTAL %s: canli alim gerceklesmedi", pair.name)
            kritik_uyari("GIRIS IPTAL", f"giris:v7new:{pair.name}", f"V7NEW {pair.name}: canli alim gerceklesmedi (broker fail)")
            return False
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        aday_paylastir.kaydet(pair.token_address, "v7new", pair.name)
        log.warning("V7NEW BUY %s $%.2f @ %.8g (h1 %.1f%%, liq $%.0f, yas %s)",
                    pair.name, usd, eff_price, pair.chg_h1, pair.liquidity_usd, yas_str(pair.pool_created_at))
        notify("[V7NEW] ALIM: %s $%.2f @ %.8g (h1 %%%.1f, liq $%.0f)"
    ...
    def _eval_position(self, pos: dict, price: float, now: float,
        ...
        price, ariza = guard_price(pos, price, now, "V7NEW", liquidity_usd=liquidity_usd)
        if ariza:
            return None
    ...
    def _fiyat_tazelendi(self, pos: dict, now: float) -> None:
        ...
        if pos.pop("kor_fiyat", None):
            ...
            log.warning("V7NEW kor fiyat sona erdi %s", pos["pair"])
            ...
            if price is None or price <= 0:
                ...
                if taze_yas >= KOR_FIYAT_SEC:
                    ...
                    if now - pos.get("_kor_alarm_ts", 0.0) >= KOR_ALARM_ARALIK_SEC:
                        ...
                        log.critical(
                            "V7NEW KOR FIYAT %s: %.0fs'dir taze fiyat yok "
    ...
    def _close_position(self, pos: dict, price: float, reason: str, now: float) -> None:
        ...
        for i in range(deneme):
            ...
            if devam:
                break
            if i + 1 < deneme:
                log.warning("V7NEW SATIS TEKRAR %s (%s): deneme %d/%d basarisiz",
                            pos["pair"], reason, i + 1, deneme)
                time.sleep(STOP_RETRY_SEC)
        if not devam:
            pos["_sat_bekle_ts"] = time.time() + SAT_COOLDOWN_SEC
            log.error("V7NEW SATIS ERTELENDI %s: canli satis gerceklesmedi", pos["pair"])
            kritik_uyari("SATIS ERTELENDI", f"sat:v7new:{pos['pair']}", f"V7NEW {pos['pair']}: canli satis fail, retry cooldown")
            return
        if canli is not None and canli.fiyat > 0:
            eff_price = canli.fiyat
        ...
        log.warning("V7NEW SELL %s pnl $%.2f (%.2f%%) — %s, hold %.0fs (mfe %.1f%% mae %.1f%%)",
                    pos["pair"], pnl, pnl_pct, reason, hold_sec, pos["mfe_pct"], pos["mae_pct"])
        notify("[V7NEW] SATIM: %s pnl $%.2f (%%%.2f) — %s, hold %.0fdk"
