...
SOL_GIRIS_MIN = float(os.getenv("R2_SOL_GIRIS_MIN", "0.0"))
STOP_RETRY_ADET = 3
STOP_RETRY_SEC = 3.0
...
class R2Engine:
    ...
    def _enter(self, client: httpx.Client) -> None:
        ...
        if SOL_GIRIS_MIN > -999 and sol_h1 is None:
            rejim_reject_kaydet(cands, "R2", None)
            if not self._regime_logged:
                self._regime_logged = True
                log.warning("R2: SOL rejim verisi yok (fail-closed), "
                            "yeni giris yok")
            return
        if SOL_GIRIS_MIN > -999 and sol_h1 is not None and sol_h1 < SOL_GIRIS_MIN:
            rejim_reject_kaydet(cands, "R2", sol_h1)
            if not self._regime_logged:
                self._regime_logged = True
                log.warning("R2: SOL- rejim kapisi KAPALI (sol_h1 %.2f < %.2f), "
                            "yeni giris yok", sol_h1, SOL_GIRIS_MIN)
            return
        if self._regime_logged:
            self._regime_logged = False
            log.warning("R2: rejim kapisi acildi (sol_h1 %s)", sol_h1)
