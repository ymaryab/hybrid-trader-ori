"""V3 senaryo motoru — v2'nin 5 kanıta dayalı düzeltmeli sanal kopyası.

v2 motoruna ve gölge senaryoya SIFIR dokunuş. Sadece şu dosyalara yazar:
  data/v3_state.json   (sanal bakiye + açık pozisyonlar)
  data/v3_trades.jsonl (her sanal kapanışta kayıt)

V3 = v2'nin TÜM kuralları AYNEN (m5>0, liq>=40k, safety, 5 slot, eşit
bölüşüm, stop_2, be arm +3, trail 5/-3, timeout_60, stop cooldown 60dk)
+ SADECE şu 5 fark (dayanak: 2026-07-02 derin analiz, 139 işlem):
  F1 h1 üst sınırı 50 yerine 15 (h1>=15 girişler net -$392, win %7-25).
  F2 aday sıralaması en DÜŞÜK chg_h1 önce (kazanan medyan h1 7.5,
     kaybeden 11.6; "en güçlü önce" en kötü bandı öne alıyordu).
  F3 rejim eşiği sol_chg_h1 < 0.5 iken giriş yok (v2'de 0; <0.5 dilimleri
     net -$238, >=0.5 net +$253).
  F4 karlı/nötr çıkış cooldown'u 45dk (v2'de 15dk; 15dk'dan geri girişler
     net -$87).
  F5 breakeven tetiği giriş+%1.5 (v2'de +%0.75; 6/6 breakeven çıkışı
     slipaj+gas sonrası negatif kapandı).

Fill'ler sanal: gerçek fiyat + v2'nin likidite-slippage modeli + gas.
Kadans: v2 ile AYNI (SCAN_INTERVAL_SEC); çeyrek interval faz kaydırmalı.
V3_ENABLED=0 ile kapatılır (panel startup'ında kontrol edilir).
"""
...
# ---- V3 eşikleri (v2 ile paylaşılanlar momentum_session'dan import edilir) ----
CHG_H1_MIN = float(os.getenv("V3_CHG_H1_MIN", "5"))      # v2 ile aynı alt sınır
CHG_H1_MAX = float(os.getenv("V3_CHG_H1_MAX", "15"))     # FARK 1: 50 -> 15
CHG_M5_MIN = float(os.getenv("V3_CHG_M5_MIN", "0"))      # v2 ile aynı (m5 > 0)
LIQ_MIN_USD = float(os.getenv("MOM_LIQ_MIN_USD", "40000"))  # v2 ile aynı taban
MAX_SLOTS = 5
START_BALANCE = float(os.getenv("V3_START_BALANCE", "1000"))
STOP_PCT = -2.0         # v2 ile birebir
BE_ARM_PCT = 3.0        # v2 ile birebir
BE_STOP_PCT = 1.5       # FARK 5: 0.75 -> 1.5 (slipaj+gas'ı karşılasın)
TRAIL_ARM_PCT = 5.0     # v2 ile birebir
TRAIL_PCT = 3.0         # v2 ile birebir
CEILING_SEC = 60 * 60   # v2 ile birebir
SOL_H1_MIN = float(os.getenv("V3_SOL_H1_MIN", "0.5"))    # FARK 3: 0 -> 0.5
COOLDOWN_STOP_SEC = float(os.getenv("MOM_COOLDOWN_STOP_MIN", "60")) * 60
COOLDOWN_EXIT_SEC = float(os.getenv("V3_COOLDOWN_EXIT_MIN", "45")) * 60  # FARK 4

STATE_FILE = "v3_state.json"
TRADES_FILE = "v3_trades.jsonl"


class V3Engine:
    """Sanal senaryo motoru. Kendi dosyaları, v2 ve gölgeye sıfır dokunuş."""
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            try:
                p.rename(backup)
                log.critical("v3 state bozuk, yedeğe taşındı: %s", backup)
            except OSError:
                log.critical("v3 state bozuk ve yedeklenemedi, temiz başlanıyor")
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            fh.close()
            log.critical("V3: başka bir instance çalışıyor, motor başlatılmıyor")
            return False
    ...
    # ---- Ana döngü (v2 ile aynı kadans, çeyrek interval faz kaydırma) --------
    def run_forever(self) -> None:
        if not self._acquire_lock():
            return
        log.warning(
            "V3 senaryo başladı — sanal $%.2f · slot %d · liq>=$%.0f · m5>%.0f · "
            "h1 %.0f..%.0f (düşük önce) · rejim>=%.1f · be+%.0f%%→+%.1f%% · "
            "cooldown stop %dm / diğer %dm",
            self.balance, MAX_SLOTS, LIQ_MIN_USD, CHG_M5_MIN, CHG_H1_MIN, CHG_H1_MAX,
            SOL_H1_MIN, BE_ARM_PCT, BE_STOP_PCT,
            COOLDOWN_STOP_SEC // 60, COOLDOWN_EXIT_SEC // 60,
        )
        self._save()
        time.sleep(SCAN_INTERVAL_SEC / 4)  # v2 (0) ve gölgeden (1/2) faz kaydır
        while True:
            try:
                self.tick()
            except Exception:
                log.exception("v3 tick hatası")
    ...
    # ---- Rejim (v2 ile aynı mantık, FARK 3: eşik 0.5, kendi cache'i) ----------
    ...
    # ---- Giriş (v2 filtreleri + FARK 1/2/3) -----------------------------------
    def _enter(self, client: httpx.Client) -> None:
        ...
        try:
            ...
            log.debug("v3 scan hatası", exc_info=True)
            return
        ...
        for pr in pairs:
            ...
            if getattr(pr, "chg_m5", 0.0) <= CHG_M5_MIN:
                continue
            if not (CHG_H1_MIN <= getattr(pr, "chg_h1", 0.0) <= CHG_H1_MAX):  # FARK 1
                continue
            cands.append(pr)
        cands.sort(key=lambda pr: pr.chg_h1)  # FARK 2: taze hareket (düşük h1) önce
        if not cands:
            return
        # Rejim filtresi: FARK 3 eşik 0.5, fail-open v2 ile aynı
        sol_h1 = None
        try:
            sol_h1 = self._sol_chg_h1(client)
        except Exception:
            log.debug("v3 rejim: sol_chg_h1 alınamadı, filtre atlandı", exc_info=True)
        if sol_h1 is not None and sol_h1 < SOL_H1_MIN:
            if not self._regime_logged:
                self._regime_logged = True
                log.warning("V3 REJIM: sol_chg_h1 %.2f%% < %.2f%%, giriş yok", sol_h1, SOL_H1_MIN)
            return
    ...
    def _open_position(self, pair, usd: float, sol_h1: float | None = None) -> bool:
        ...
        self._save()
        log.warning("V3 BUY %s $%.2f @ %.8g (m5 %.1f%%, h1 %.1f%%, liq $%.0f)",
                    pair.name, usd, eff_price, pos["chg_m5"], pair.chg_h1,
                    pair.liquidity_usd)
        return True

    # ---- Çıkış (v2 state machine birebir; FARK 5: be tetiği +%1.5) ------------
    ...
    def _close_position(self, pos: dict, price: float, reason: str, now: float) -> None:
        ...
        # FARK 4: stop 60dk (v2 ile aynı), diğer tüm sebepler 45dk (v2'de 15dk)
        ...
        try:
            ...
            pass
        self._save()
        log.warning("V3 SELL %s pnl $%.2f (%.2f%%) — %s, hold %.0fs (mfe %.1f%% mae %.1f%%)",
