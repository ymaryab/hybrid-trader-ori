"""V7M "Major" senaryo motoru — h1 10-30, liq>=$300k, TP+%2, timeout 90dk — 15dk kisa pencere + felaket freni (canli hat).

Diğer motorlara (v2/v3/v4/v5/v6/gölge) SIFIR dokunuş. Sadece şu dosyalara yazar:
  data/v7m_state.json   (sanal bakiye + açık pozisyonlar)
  data/v7m_trades.jsonl (her sanal kapanışta kayıt)

V7M = V6 tabanli; 15 Tem kullanici karari: sabir ve tavan 120 dk -> 15 dk,
-%10 felaket freni geri geldi (v7c/golge senkron degil, canli hat once).
  GİRİŞ : liq >= $150k VE 10 <= chg_h1 <= 50 (yalin bant, kacinma kapali).
  ÇIKIŞ : tp_2 (+%2 UZERI) / stop_felaket (-%15 her an) /
          stop_gec (30dk sabir sonrasi -%2) / timeout_60 (60dk kosulsuz).
  REJİM : sol_chg_h1 >= 0.35 (16 Tem: 0.5 -> 0.35 geri). Env: V7M_SOL_H1_MIN.
...
v7m:3/8, v4:3/4; 12 Tem: v7m canlı para taşıdığı için v6 ile faz takası
yapıldı, v7m erken v6 geç). V7M_ENABLED=0 ile kapatılır.
"""
...
# ---- V7M eşikleri (v6 zemini, 15dk kisa pencere + felaket freni) ---------------
CHG_H1_MIN = float(os.getenv("V7M_CHG_H1_MIN", "10"))
CHG_H1_MAX = float(os.getenv("V7M_CHG_H1_MAX", "30"))
LIQ_MIN_USD = float(os.getenv("V7M_LIQ_MIN_USD", "200000"))
MAX_SLOTS = 5
START_BALANCE = float(os.getenv("V7M_START_BALANCE", "1000"))
TP_PCT = 2.0            # giris +%2
GRACE_SEC = 30 * 60     # 30dk sabir (15 Tem kullanici karari)
LATE_STOP_PCT = -3.0    # 30dk sonrasi -%3 alti: girisin -%2 alti SAT
CEILING_SEC = 90 * 60   # 90dk tavan (15 Tem kullanici karari)
DISASTER_PCT = -12.0    # HER AN -%15'a dokunursa derhal sat
SOL_H1_MIN = float(os.getenv("V7M_SOL_H1_MIN", "0.35"))  # 16 Tem kullanici karari: 0.5 -> 0.35 geri
# h1 bant kacinma: yalin kural setinde kapali (LO=HI=0 -> h1_bant_atla False doner).
H1_SKIP_LO = float(os.getenv("V7M_H1_SKIP_LO", "0"))
H1_SKIP_HI = float(os.getenv("V7M_H1_SKIP_HI", "0"))
...
H1_SKIP_M5_KOSUL = os.getenv("V7M_H1_SKIP_M5_KOSUL", "1").strip() != "0"
...
EXIT_SLIPPAGE_BPS = {"tp_2": 150, "timeout_90": 150, "stop_gec": 300, "stop_felaket": 1000}
...
STATE_FILE = "v7m_state.json"
TRADES_FILE = "v7m_trades.jsonl"
...
def h1_bant_atla(chg_h1: float, chg_m5: float | None = None) -> bool:
    ...
    V7M_H1_SKIP_M5_KOSUL=0 ile kapatilir. m5 bilinmiyorsa (None) eski
...
class V7MEngine:
    ...
    def __init__(self, settings) -> None:
        ...
        self._huni = HuniSayac("V7M")
        self._lock_fh = None
        self._son_exec_neden: str | None = None
        self._belirsiz_aday: dict | None = None    # belirsiz alim: benimseme bekleyen aday
        # 16 Tem: CANLI_MOTOR env swap altyapisi. CANLI_MOTOR=v7m (default) ise
        # BROKER_MODE'a gore live/paper/dryrun; diger motor secildiyse V7M paper'a
        # duser (canli hat baska motora gecmis). Broker init hatasinda fail-closed.
        self._exec, self._exec_arizali = init_motor_exec("v7m")
        self._load()
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            try:
                p.rename(backup)
                log.critical("v7m state bozuk, yedeğe taşındı: %s", backup)
            except OSError:
                log.critical("v7m state bozuk ve yedeklenemedi, temiz başlanıyor")
    ...
    def _restore_day_realized(self) -> None:
        ...
        try:
            ...
            log.debug("V7M gun ici pnl geri yuklenemedi", exc_info=True)
    ...
    def _entries_blocked(self) -> str | None:
        ...
        if kill_is_active():
            if not self._kill_logged:
                ...
                log.critical("V7M: kill-switch AKTIF, yeni girisler durdu (cikislar suruyor)")
            return "kill_switch"
        if self._kill_logged:
            self._kill_logged = False
            log.warning("V7M: kill-switch kalkti, girisler serbest")
    ...
    def _canli_mtm(self) -> float | None:
        try:
            ...
            log.debug("V7M canli MTM okunamadi", exc_info=True)
        return None
    ...
    def _gun_limiti(self) -> tuple[float | None, bool]:
        ...
        log.warning("V7M gun limiti sabitlendi: MTM $%.2f x %%%g = $%.2f%s",
    ...
    def _exec_fill(self, yon: str, token_address: str, *, usd: float = 0.0,
        ...
        try:
            fill = self._exec.execute(ExecOrder(
                engine="V7M", yon=yon, token_address=token_address,
                usd=usd, amount_token=amount_token, ref_fiyat=ref_fiyat,
                slippage_bps=slippage_bps, acilis_ts=acilis_ts))
        except Exception as e:
            log.error("V7M yurutme hatasi (%s %s): %s", yon, token_address[:8], e)
            fill = None
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            fh.close()
            log.critical("V7M: başka bir instance çalışıyor, motor başlatılmıyor")
            return False
    ...
    def run_forever(self) -> None:
        if not self._acquire_lock():
            return
        log.warning(
            "V7M senaryo basladi (30dk sabir + 60dk tavan + felaket freni) - sanal $%.2f · "
            "slot %d · giris liq>=$%.0f + h1 %.0f..%.0f (skip %.0f..%.0f) · rejim>=%.2f · "
            "cikis tp+%.0f%% uzeri / felaket %%%.0f / "
        ...
        while True:
            ...
            while True:
                ...
                try:
                    ...
                    log.exception("v7m hizli cikis hatası")
    ...
    def _belirsiz_takip(self) -> None:
        ...
        if durum == "gerceklesti" and detay and detay.get("fiyat", 0) > 0:
            ...
            log.warning("V7M BELIRSIZ SONUC %s: tx zincirde yok, para cikmadi, "
                        "giris kayitsiz iptal", aday["pair"])
        else:  # cozulemedi / kayit_yok / bozuk detay
            log.critical("V7M BELIRSIZ SONUC %s: cozulemedi (%s); kilit kapali "
    ...
    def _belirsiz_pozisyon_ac(self, aday: dict, detay: dict) -> None:
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        log.warning("V7M BUY (mutabakat) %s $%.2f @ %.8g: belirsiz tx zincirde "
                    "gerceklesti, pozisyon benimsendi (tx %s)",
                    aday["pair"], usd, entry, detay["tx_id"])
        notify("[V7M] ALIM (mutabakat): %s $%.2f @ %.8g — belirsiz tx zincirde gerceklesti"
               % (aday["pair"], usd, entry))
    ...
    def _enter(self, client: httpx.Client) -> None:
        ...
        try:
            ...
            log.warning("V7M giris tick atlandi, tarama hatasi: %r", e)
            return
        ...
        for pr in pairs:
            ...
            _izin, _red_nedeni = aday_paylastir.iddia_et(pr.token_address, "v7m", pr.name)
            if not _izin:
                continue
        ...
        if sol_h1 is None:
            if not self._regime_logged:
                ...
                log.warning("V7M REJIM: sol_h1 verisi yok (fail-closed), giriş kapalı")
            rejim_reject_kaydet(cands, "V7M", None)
            return
        if sol_h1 < SOL_H1_MIN:
            if not self._regime_logged:
                self._regime_logged = True
                log.warning("V7M REJIM: sol_chg_h1 %.2f%% < %.2f%%, giriş yok", sol_h1, SOL_H1_MIN)
            rejim_reject_kaydet(cands, "V7M", sol_h1)
            return
        ...
        for pair in cands:
            ...
            try:
                ...
                safety_reject_kaydet(pair, "V7M", "safety_hata", type(e).__name__)
                continue
            time.sleep(0.2 if self._aggressive else 1.5)
            if not report.ok:
                safety_reject_kaydet(
                    pair, "V7M", report.kapi or "safety_red", "; ".join(report.reasons[:2])
                )
                continue
    ...
    def _bant_skip_kaydet(self, pair, now: float) -> None:
        ...
        bant_reject_kaydet(pair, "V7M", sol_h1)
    ...
    def _open_position(self, pair, usd: float, sol_h1: float | None = None,
        ...
        if self.balance < usd + gas:
            return False
        taze = taze_teyit(pair, "V7M", client)
        if taze.iptal:
            log.warning("V7M GIRIS IPTAL %s: taze fiyat taramanin %%%.2f ustunde (kaynak %s)",
        ...
        if not devam:
            if self._son_exec_neden == "islem_belirsiz":
                ...
                log.critical("V7M GIRIS BELIRSIZ %s: zincir mutabakati "
                             "bekleniyor, sonuca kadar canli islemler kilitli",
                             pair.name)
                return False
            log.error("V7M GIRIS IPTAL %s: canli alim gerceklesmedi", pair.name)
            kritik_uyari("GIRIS IPTAL", f"giris:v7m:{pair.name}", f"V7M {pair.name}: canli alim gerceklesmedi (broker fail)")
            return False
    ...
    def _eval_position(self, pos: dict, price: float, now: float,
        ...
        price, ariza = guard_price(pos, price, now, "V7M", liquidity_usd=liquidity_usd)
        if ariza:
    ...
    def _fiyat_tazelendi(self, pos: dict, now: float) -> None:
        ...
        if pos.pop("kor_fiyat", None):
            ...
            log.warning("V7M kor fiyat sona erdi %s: taze fiyat geri geldi",
                        pos["pair"])
            ...
            if price is None or price <= 0:
                ...
                if taze_yas >= KOR_FIYAT_SEC:
                    ...
                    if now - pos.get("_kor_alarm_ts", 0.0) >= KOR_ALARM_ARALIK_SEC:
                        ...
                        log.critical(
                            "V7M KOR FIYAT %s: %.0fs'dir taze fiyat yok, "
                            "degerleme donuk (last %.8g); stoplar "
                            "tetiklenemiyor olabilir", pos["pair"],
                            taze_yas, price)
                        kritik_uyari("KOR FIYAT", f"kor:v7m:{pos['pair']}", f"V7M {pos['pair']}: {taze_yas:.0f}s taze fiyat yok, stop tetiklenemez")
            else:
    ...
    def _close_position(self, pos: dict, price: float, reason: str, now: float) -> None:
        ...
        for i in range(deneme):
            ...
            if devam:
                break
            if i + 1 < deneme:
                log.warning("V7M SATIS TEKRAR %s (%s): deneme %d/%d basarisiz, "
        ...
        if not devam:
            ...
            log.error("V7M SATIS ERTELENDI %s: canli satis gerceklesmedi, "
                      "%.0fs soguma sonrasi tekrar denenecek",
                      pos["pair"], SAT_COOLDOWN_SEC)
            kritik_uyari("SATIS ERTELENDI", f"sat:v7m:{pos['pair']}", f"V7M {pos['pair']}: canli satis fail, retry cooldown")
            return
        ...
        log.warning("V7M SELL %s pnl $%.2f (%.2f%%) — %s, hold %.0fs (mfe %.1f%% mae %.1f%%)",
                    pos["pair"], pnl, pnl_pct, reason, hold_sec, pos["mfe_pct"], pos["mae_pct"])
        notify("[V7M] SATIM: %s pnl $%.2f (%%%.2f) — %s, hold %.0fdk"
