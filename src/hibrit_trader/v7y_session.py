"""V7Y "Snipers" paper motoru — h1 40-50 dar bant scalp (retro %92 winrate bandı) (V7Y klonu, 16 Tem: V6 yerine listelendi).
...
V7Y_ENABLED=0 ile kapatilir.

Diger motorlara SIFIR dokunus. Sadece su dosyalara yazar:
  data/v7y_state.json   (sanal bakiye + acik pozisyonlar)
  data/v7y_trades.jsonl (her sanal kapanista kayit)
"""
...
# ---- V7Y esikleri (kullanici kural seti 2026-07-15) --------------------
CHG_H1_MIN = float(os.getenv("V7Y_CHG_H1_MIN", "40"))
CHG_H1_MAX = float(os.getenv("V7Y_CHG_H1_MAX", "50"))
LIQ_MIN_USD = float(os.getenv("V7Y_LIQ_MIN_USD", "100000"))
MAX_SLOTS = 5
START_BALANCE = float(os.getenv("V7Y_START_BALANCE", "1000"))
TP_PCT = float(os.getenv("V7Y_TP_PCT", "5.0"))
SOL_H1_MIN = float(os.getenv("V7Y_SOL_H1_MIN", "0.35"))
...
EXIT_SLIPPAGE_BPS = {"tp_2_partial": 150, "runner_trail": 150,
...
# Trail-arm parametreleri (V7Y ekstra):
# TP hit'te kismi_orani kadari satilir (default %50), kalan runner.
# Runner mode: peak izle, peak*(1-trail_pct/100) alti tetigi ile kalani sat.
KISMI_ORAN = float(os.getenv("V7Y_KISMI_ORAN", "0.5"))  # 0.5 = yarisi
TRAIL_PCT = float(os.getenv("V7Y_TRAIL_PCT", "10.0"))    # tepeden %10 asagi

# V7Y ozel koruma katmani (h1 40-50 volatil aday)
DISASTER_PCT = float(os.getenv("V7Y_DISASTER_PCT", "-10.0"))
GRACE_SEC = int(os.getenv("V7Y_GRACE_SEC", str(10 * 60)))
LATE_STOP_PCT = float(os.getenv("V7Y_LATE_STOP_PCT", "-3.0"))
CEILING_SEC = int(os.getenv("V7Y_CEILING_SEC", str(30 * 60)))
M5_MIN = float(os.getenv("V7Y_M5_MIN", "0.0"))

STATE_FILE = "v7y_state.json"
TRADES_FILE = "v7y_trades.jsonl"
...
class V7YEngine:
    ...
    def __init__(self, settings) -> None:
        ...
        self._huni = HuniSayac("V7Y")
        self._lock_fh = None
        self._son_exec_neden: str | None = None
        self._belirsiz_aday: dict | None = None
        # 16 Tem: CANLI_MOTOR env swap altyapisi. Default paper; CANLI_MOTOR=v7y
        # secilirse make_exec_broker (live/dryrun) devreye girer.
        self._exec, self._exec_arizali = init_motor_exec("v7y")
        self._load()
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            try:
                p.rename(backup)
                log.critical("v7y state bozuk, yedege tasindi: %s", backup)
            except OSError:
                log.critical("v7y state bozuk ve yedeklenemedi, temiz baslaniyor")
    ...
    def _restore_day_realized(self) -> None:
        ...
        try:
            ...
            log.debug("V7Y gun ici pnl geri yuklenemedi", exc_info=True)
    ...
    def _entries_blocked(self) -> str | None:
        ...
        if kill_is_active():
            if not self._kill_logged:
                ...
                log.critical("V7Y: kill-switch AKTIF, yeni girisler durdu (cikislar suruyor)")
            return "kill_switch"
        if self._kill_logged:
            self._kill_logged = False
            log.warning("V7Y: kill-switch kalkti, girisler serbest")
        if DAILY_LOSS_LIMIT_USD > 0 or self._pct_limit_aktif():
            ...
            if limit is None and not kesin:
                if not self._limit_belirsiz_logged:
                    ...
                    log.critical("V7Y: gun limiti hesaplanamadi, yeni giris kapali (fail-closed)")
                return "daily_limit_belirsiz"
            if self._limit_belirsiz_logged:
                self._limit_belirsiz_logged = False
                log.warning("V7Y: gun limiti hesaplandi, belirsizlik kalkti")
            if limit is not None and self._day_realized <= -limit:
                if not self._limit_logged:
                    self._limit_logged = True
                    log.critical(
                        "V7Y: gunluk zarar limiti asildi ($%.2f <= -$%.2f), "
    ...
    def _canli_mtm(self) -> float | None:
        try:
            ...
            log.debug("V7Y canli MTM okunamadi", exc_info=True)
        return None
    ...
    def _gun_limiti(self) -> tuple[float | None, bool]:
        ...
        self._save()
        log.warning("V7Y gun limiti sabitlendi: MTM $%.2f x %%%g = $%.2f",
    ...
    def _exec_fill(self, yon: str, token_address: str, *, usd: float = 0.0,
        ...
        try:
            ...
            log.error("V7Y yurutme hatasi (%s %s): %s", yon, token_address[:8], e)
            fill = None
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            fh.close()
            log.critical("V7Y: baska bir instance calisiyor, motor baslatilmiyor")
            return False
    ...
    def run_forever(self) -> None:
        if not self._acquire_lock():
            return
        log.warning(
            "V7Y paper basladi (TP=+%%%.1f tek cikis, stop yok) - sanal $%.2f · "
        ...
        # v7d 7/8 kullaniyor, v7 1/1, momentum 5/8, v6 farkli. v7y faz: 6/8.
        time.sleep(SCAN_INTERVAL_SEC * 6 / 8)
        while True:
            try:
                self.tick()
            except Exception:
                log.exception("v7y tick hatasi")
            ...
            while True:
                ...
                try:
                    ...
                    log.exception("v7y hizli cikis hatasi")
    ...
    def _belirsiz_takip(self) -> None:
        ...
        if durum == "gerceklesti" and detay and detay.get("fiyat", 0) > 0:
            ...
            log.warning("V7Y BELIRSIZ SONUC %s: tx zincirde yok, iptal", aday["pair"])
        else:
            log.critical("V7Y BELIRSIZ SONUC %s: cozulemedi (%s)", aday["pair"], durum)
    ...
    def _belirsiz_pozisyon_ac(self, aday: dict, detay: dict) -> None:
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        log.warning("V7Y BUY (mutabakat) %s $%.2f @ %.8g", aday["pair"], usd, entry)
    ...
    def _enter(self, client: httpx.Client) -> None:
        ...
        try:
            ...
            log.warning("V7Y giris tick atlandi, tarama hatasi: %r", e)
            return
        ...
        for pr in pairs:
            ...
            _izin, _red_nedeni = aday_paylastir.iddia_et(pr.token_address, "v7y", pr.name)
            if not _izin:
                continue
            ...
            if not (CHG_H1_MIN <= h1 <= CHG_H1_MAX):
                continue
            # V7Y: m5>0 zorunlu (tepede yorgun eleme)
            if (getattr(pr, "chg_m5", 0) or 0) <= M5_MIN:
                continue
            cands.append(pr)
        ...
        for pair in cands:
            ...
            try:
                ...
                safety_reject_kaydet(pair, "V7Y", "safety_hata", type(e).__name__)
                continue
            time.sleep(0.2 if self._aggressive else 1.5)
            if not report.ok:
                safety_reject_kaydet(
                    pair, "V7Y", report.kapi or "safety_red",
    ...
    def _open_position(self, pair, usd: float, sol_h1: float | None = None,
        ...
        if self.balance < usd + gas:
            return False
        taze = taze_teyit(pair, "V7Y", client)
        if taze.iptal:
            log.warning("V7Y GIRIS IPTAL %s: taze fiyat taramanin %%%.2f ustunde (kaynak %s)",
        ...
        if not devam:
            if self._son_exec_neden == "islem_belirsiz":
                ...
                log.critical("V7Y GIRIS BELIRSIZ %s: zincir mutabakati bekleniyor",
                             pair.name)
                return False
            log.error("V7Y GIRIS IPTAL %s: canli alim gerceklesmedi", pair.name)
            kritik_uyari("GIRIS IPTAL", f"giris:v7y:{pair.name}", f"V7Y {pair.name}: canli alim gerceklesmedi (broker fail)")
            return False
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        aday_paylastir.kaydet(pair.token_address, "v7y", pair.name)
        log.warning("V7Y BUY %s $%.2f @ %.8g (h1 %.1f%%, liq $%.0f, yas %s)",
                    pair.name, usd, eff_price, pair.chg_h1, pair.liquidity_usd, yas_str(pair.pool_created_at))
        notify("[V7Y] ALIM: %s $%.2f @ %.8g (h1 %%%.1f, liq $%.0f)"
    ...
    def _eval_position(self, pos: dict, price: float, now: float,
        ...
        price, ariza = guard_price(pos, price, now, "V7Y", liquidity_usd=liquidity_usd)
        if ariza:
            return None
        ...
        if pnl_pct <= DISASTER_PCT:
            return "stop_felaket"
        # 2) runner_mode aktifse peak izle + trail cikis
        if pos.get("runner_mode"):
            ...
            if price <= peak * (1 - TRAIL_PCT / 100.0):
                return "runner_trail"
            return None
        # 3) TP+%5 hit -> kismi sat + kalani runner
        if pnl_pct > TP_PCT:
            return "tp_2_partial"
        # 4) grace 10dk sonrasi -%3 alti stop
    ...
    def _fiyat_tazelendi(self, pos: dict, now: float) -> None:
        ...
        if pos.pop("kor_fiyat", None):
            ...
            log.warning("V7Y kor fiyat sona erdi %s", pos["pair"])
            ...
            if price is None or price <= 0:
                ...
                if taze_yas >= KOR_FIYAT_SEC:
                    ...
                    if now - pos.get("_kor_alarm_ts", 0.0) >= KOR_ALARM_ARALIK_SEC:
                        ...
                        log.critical(
                            "V7Y KOR FIYAT %s: %.0fs'dir taze fiyat yok "
    ...
    def _close_position(self, pos: dict, price: float, reason: str, now: float) -> None:
        if time.time() < pos.get("_sat_bekle_ts", 0.0):
            return
        # Kismi satis: reason=tp_2_partial -> yarisini sat, kalani runner
        kismi = (reason == "tp_2_partial")
        cost = pos["cost_usd"]
        satilan_oran = KISMI_ORAN if kismi else 1.0
        ...
        for i in range(deneme):
            ...
            if devam:
                break
            if i + 1 < deneme:
                log.warning("V7Y SATIS TEKRAR %s (%s): deneme %d/%d basarisiz",
                            pos["pair"], reason, i + 1, deneme)
                time.sleep(STOP_RETRY_SEC)
        if not devam:
            pos["_sat_bekle_ts"] = time.time() + SAT_COOLDOWN_SEC
            log.error("V7Y SATIS ERTELENDI %s: canli satis gerceklesmedi", pos["pair"])
            kritik_uyari("SATIS ERTELENDI", f"sat:v7y:{pos['pair']}", f"V7Y {pos['pair']}: canli satis fail, retry cooldown")
            return
        if canli is not None and canli.fiyat > 0:
            eff_price = canli.fiyat
        ...
        # Kismi satis ise: pos'ta kalani birak, runner_mode aktive et
        if kismi:
            ...
            self._save()
            log.warning("V7Y PARTIAL %s: yarisi satildi pnl $%.2f (%.2f%%), "
                        "runner devam (peak=%.8g, trail=%.1f%%)",
                        pos["pair"], pnl, pnl_pct, pos["runner_peak"], TRAIL_PCT)
            notify("[V7Y] PARTIAL: %s +$%.2f (%%%.2f) — kalani runner (peak %.8g)"
        ...
        log.warning("V7Y SELL %s pnl $%.2f (%.2f%%) — %s, hold %.0fs (mfe %.1f%% mae %.1f%%)",
                    pos["pair"], pnl, pnl_pct, reason, hold_sec, pos["mfe_pct"], pos["mae_pct"])
        notify("[V7Y] SATIM: %s pnl $%.2f (%%%.2f) — %s, hold %.0fdk"
