"""YZ motoru — haftalarin verisinden damitilmis canli cekirdek (21 Tem).

Uc parca, ucu de olculmus:
  KENAR : v7hizli bandi (5<=h1<=45, liq>=100k) + TP+2 tek cikis.
...
# ---- YZ esikleri (kullanici kural seti 2026-07-15) --------------------
CHG_H1_MIN = float(os.getenv("YZ_CHG_H1_MIN", "5"))
CHG_H1_MAX = float(os.getenv("YZ_CHG_H1_MAX", "45"))
LIQ_MIN_USD = float(os.getenv("YZ_LIQ_MIN_USD", "100000"))
MAX_SLOTS = 5
START_BALANCE = float(os.getenv("YZ_START_BALANCE", "1000"))
TP_PCT = float(os.getenv("YZ_TP_PCT", "2.0"))
SOL_H1_MIN = float(os.getenv("YZ_SOL_H1_MIN", "0.35"))
...
TIMEOUT_MIN = float(os.getenv("YZ_TIMEOUT_MIN", "60"))
...
class YZEngine:
    ...
    def __init__(self, settings) -> None:
        ...
        self._huni = HuniSayac("YZ")
        self._lock_fh = None
        self._son_exec_neden: str | None = None
        self._belirsiz_aday: dict | None = None
        # 16 Tem: CANLI_MOTOR env swap altyapisi. Default paper; CANLI_MOTOR=yz
        # secilirse make_exec_broker (live/dryrun) devreye girer.
        self._exec, self._exec_arizali = init_motor_exec("yz")
        self._load()
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            try:
                p.rename(backup)
                log.critical("yz state bozuk, yedege tasindi: %s", backup)
            except OSError:
                log.critical("yz state bozuk ve yedeklenemedi, temiz baslaniyor")
    ...
    def _restore_day_realized(self) -> None:
        ...
        try:
            ...
            log.debug("YZ gun ici pnl geri yuklenemedi", exc_info=True)
    ...
    def _entries_blocked(self) -> str | None:
        ...
        if kill_is_active():
            if not self._kill_logged:
                ...
                log.critical("YZ: kill-switch AKTIF, yeni girisler durdu (cikislar suruyor)")
            return "kill_switch"
        if self._kill_logged:
            self._kill_logged = False
            log.warning("YZ: kill-switch kalkti, girisler serbest")
        if DAILY_LOSS_LIMIT_USD > 0 or self._pct_limit_aktif():
            ...
            if limit is None and not kesin:
                if not self._limit_belirsiz_logged:
                    ...
                    log.critical("YZ: gun limiti hesaplanamadi, yeni giris kapali (fail-closed)")
                return "daily_limit_belirsiz"
            if self._limit_belirsiz_logged:
                self._limit_belirsiz_logged = False
                log.warning("YZ: gun limiti hesaplandi, belirsizlik kalkti")
            if limit is not None and self._day_realized <= -limit:
                if not self._limit_logged:
                    self._limit_logged = True
                    log.critical(
                        "YZ: gunluk zarar limiti asildi ($%.2f <= -$%.2f), "
    ...
    def _canli_mtm(self) -> float | None:
        try:
            ...
            log.debug("YZ canli MTM okunamadi", exc_info=True)
        return None
    ...
    def _gun_limiti(self) -> tuple[float | None, bool]:
        ...
        self._save()
        log.warning("YZ gun limiti sabitlendi: MTM $%.2f x %%%g = $%.2f",
    ...
    def _exec_fill(self, yon: str, token_address: str, *, usd: float = 0.0,
        ...
        try:
            ...
            log.error("YZ yurutme hatasi (%s %s): %s", yon, token_address[:8], e)
            fill = None
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            fh.close()
            log.critical("YZ: baska bir instance calisiyor, motor baslatilmiyor")
            return False
    ...
    def run_forever(self) -> None:
        if not self._acquire_lock():
            return
        log.warning(
            "YZ paper basladi (TP=+%%%.1f tek cikis, stop yok) - sanal $%.2f · "
        ...
        # v7d 7/8 kullaniyor, v7 1/1, momentum 5/8, v6 farkli. yz faz: 6/8.
        time.sleep(SCAN_INTERVAL_SEC * 2 / 8)
        while True:
            try:
                self.tick()
            except Exception:
                log.exception("yz tick hatasi")
            ...
            while True:
                ...
                try:
                    ...
                    log.exception("yz hizli cikis hatasi")
    ...
    def _belirsiz_takip(self) -> None:
        ...
        if durum == "gerceklesti" and detay and detay.get("fiyat", 0) > 0:
            ...
            log.warning("YZ BELIRSIZ SONUC %s: tx zincirde yok, iptal", aday["pair"])
        else:
            log.critical("YZ BELIRSIZ SONUC %s: cozulemedi (%s)", aday["pair"], durum)
    ...
    def _belirsiz_pozisyon_ac(self, aday: dict, detay: dict) -> None:
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        log.warning("YZ BUY (mutabakat) %s $%.2f @ %.8g", aday["pair"], usd, entry)
    ...
    def _enter(self, client: httpx.Client) -> None:
        ...
        try:
            ...
            log.warning("YZ giris tick atlandi, tarama hatasi: %r", e)
            return
        ...
        for pr in pairs:
            ...
            _izin, _red_nedeni = aday_paylastir.iddia_et(pr.token_address, "yz", pr.name)
            if not _izin:
                continue
        ...
        for pair in cands:
            ...
            try:
                ...
                safety_reject_kaydet(pair, "YZ", "safety_hata", type(e).__name__)
                continue
            time.sleep(0.2 if self._aggressive else 1.5)
            if not report.ok:
                safety_reject_kaydet(
                    pair, "YZ", report.kapi or "safety_red",
    ...
    def _open_position(self, pair, usd: float, sol_h1: float | None = None,
        ...
        if self.balance < usd + gas:
            return False
        taze = taze_teyit(pair, "YZ", client)
        if taze.iptal:
            log.warning("YZ GIRIS IPTAL %s: taze fiyat taramanin %%%.2f ustunde (kaynak %s)",
        ...
        if not devam:
            ...
            "pool_yas_dk": (round((time.time() - float(pair.pool_created_at)) / 60.0, 1)
                ...
                log.critical("YZ GIRIS BELIRSIZ %s: zincir mutabakati bekleniyor",
                             pair.name)
                return False
            log.error("YZ GIRIS IPTAL %s: canli alim gerceklesmedi", pair.name)
            kritik_uyari("GIRIS IPTAL", f"giris:yz:{pair.name}", f"YZ {pair.name}: canli alim gerceklesmedi (broker fail)")
            return False
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        aday_paylastir.kaydet(pair.token_address, "yz", pair.name)
        log.warning("YZ BUY %s $%.2f @ %.8g (h1 %.1f%%, liq $%.0f, yas %s)",
                    pair.name, usd, eff_price, pair.chg_h1, pair.liquidity_usd, yas_str(pair.pool_created_at))
        notify("[YZ] ALIM: %s $%.2f @ %.8g (h1 %%%.1f, liq $%.0f)"
    ...
    def _eval_position(self, pos: dict, price: float, now: float,
        ...
        price, ariza = guard_price(pos, price, now, "YZ", liquidity_usd=liquidity_usd)
        if ariza:
            return None
    ...
    def _fiyat_tazelendi(self, pos: dict, now: float) -> None:
        ...
        if pos.pop("kor_fiyat", None):
            ...
            log.warning("YZ kor fiyat sona erdi %s", pos["pair"])
            ...
            if price is None or price <= 0:
                ...
                if taze_yas >= KOR_FIYAT_SEC:
                    ...
                    if now - pos.get("_kor_alarm_ts", 0.0) >= KOR_ALARM_ARALIK_SEC:
                        ...
                        log.critical(
                            "YZ KOR FIYAT %s: %.0fs'dir taze fiyat yok "
    ...
    def _close_position(self, pos: dict, price: float, reason: str, now: float) -> None:
        ...
        for i in range(deneme):
            ...
            if devam:
                break
            if i + 1 < deneme:
                log.warning("YZ SATIS TEKRAR %s (%s): deneme %d/%d basarisiz",
                            pos["pair"], reason, i + 1, deneme)
                time.sleep(STOP_RETRY_SEC)
        if not devam:
            pos["_sat_bekle_ts"] = time.time() + SAT_COOLDOWN_SEC
            log.error("YZ SATIS ERTELENDI %s: canli satis gerceklesmedi", pos["pair"])
            kritik_uyari("SATIS ERTELENDI", f"sat:yz:{pos['pair']}", f"YZ {pos['pair']}: canli satis fail, retry cooldown")
            return
        if canli is not None and canli.fiyat > 0:
            eff_price = canli.fiyat
        gas = GAS_COST_USD.get(pos["chain"], 0.1)
        ...
        log.warning("YZ SELL %s pnl $%.2f (%.2f%%) — %s, hold %.0fs (mfe %.1f%% mae %.1f%%)",
                    pos["pair"], pnl, pnl_pct, reason, hold_sec, pos["mfe_pct"], pos["mae_pct"])
        notify("[YZ] SATIM: %s pnl $%.2f (%%%.2f) — %s, hold %.0fdk"
