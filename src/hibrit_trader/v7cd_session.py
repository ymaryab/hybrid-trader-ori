"""V7CD paper motoru — TP=+%2 tek cikis (V7CD klonu, 16 Tem: V6 yerine listelendi).
...
V7CD_ENABLED=0 ile kapatilir.

Diger motorlara SIFIR dokunus. Sadece su dosyalara yazar:
  data/v7cd_state.json   (sanal bakiye + acik pozisyonlar)
  data/v7cd_trades.jsonl (her sanal kapanista kayit)
"""
...
# ---- V7CD esikleri (kullanici kural seti 2026-07-15) --------------------
CHG_H1_MIN = float(os.getenv("V7CD_CHG_H1_MIN", "15"))
CHG_H1_MAX = float(os.getenv("V7CD_CHG_H1_MAX", "55"))
LIQ_MIN_USD = float(os.getenv("V7CD_LIQ_MIN_USD", "100000"))
MAX_SLOTS = 5
START_BALANCE = float(os.getenv("V7CD_START_BALANCE", "1000"))
TP_PCT = float(os.getenv("V7CD_TP_PCT", "2.0"))
SOL_H1_MIN = float(os.getenv("V7CD_SOL_H1_MIN", "0.35"))
...
# Trail-arm parametreleri (V7CD ekstra):
...
KISMI_ORAN = float(os.getenv("V7CD_KISMI_ORAN", "0.5"))  # 0.5 = yarisi
TRAIL_PCT = float(os.getenv("V7CD_TRAIL_PCT", "3.0"))    # tepeden %3 asagi

STATE_FILE = "v7cd_state.json"
TRADES_FILE = "v7cd_trades.jsonl"
...
class V7CDEngine:
    ...
    def __init__(self, settings) -> None:
        ...
        self._huni = HuniSayac("V7CD")
        self._lock_fh = None
        self._son_exec_neden: str | None = None
        self._belirsiz_aday: dict | None = None
        # 16 Tem: CANLI_MOTOR env swap altyapisi. Default paper; CANLI_MOTOR=v7cd
        # secilirse make_exec_broker (live/dryrun) devreye girer.
        self._exec, self._exec_arizali = init_motor_exec("v7cd")
        self._load()
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            try:
                p.rename(backup)
                log.critical("v7cd state bozuk, yedege tasindi: %s", backup)
            except OSError:
                log.critical("v7cd state bozuk ve yedeklenemedi, temiz baslaniyor")
    ...
    def _restore_day_realized(self) -> None:
        ...
        try:
            ...
            log.debug("V7CD gun ici pnl geri yuklenemedi", exc_info=True)
    ...
    def _entries_blocked(self) -> str | None:
        ...
        if kill_is_active():
            if not self._kill_logged:
                ...
                log.critical("V7CD: kill-switch AKTIF, yeni girisler durdu (cikislar suruyor)")
            return "kill_switch"
        if self._kill_logged:
            self._kill_logged = False
            log.warning("V7CD: kill-switch kalkti, girisler serbest")
        if DAILY_LOSS_LIMIT_USD > 0 or self._pct_limit_aktif():
            ...
            if limit is None and not kesin:
                if not self._limit_belirsiz_logged:
                    ...
                    log.critical("V7CD: gun limiti hesaplanamadi, yeni giris kapali (fail-closed)")
                return "daily_limit_belirsiz"
            if self._limit_belirsiz_logged:
                self._limit_belirsiz_logged = False
                log.warning("V7CD: gun limiti hesaplandi, belirsizlik kalkti")
            if limit is not None and self._day_realized <= -limit:
                if not self._limit_logged:
                    self._limit_logged = True
                    log.critical(
                        "V7CD: gunluk zarar limiti asildi ($%.2f <= -$%.2f), "
    ...
    def _canli_mtm(self) -> float | None:
        try:
            ...
            log.debug("V7CD canli MTM okunamadi", exc_info=True)
        return None
    ...
    def _gun_limiti(self) -> tuple[float | None, bool]:
        ...
        self._save()
        log.warning("V7CD gun limiti sabitlendi: MTM $%.2f x %%%g = $%.2f",
    ...
    def _exec_fill(self, yon: str, token_address: str, *, usd: float = 0.0,
        ...
        try:
            ...
            log.error("V7CD yurutme hatasi (%s %s): %s", yon, token_address[:8], e)
            fill = None
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            fh.close()
            log.critical("V7CD: baska bir instance calisiyor, motor baslatilmiyor")
            return False
    ...
    def run_forever(self) -> None:
        if not self._acquire_lock():
            return
        log.warning(
            "V7CD paper basladi (TP=+%%%.1f tek cikis, stop yok) - sanal $%.2f · "
        ...
        # v7d 7/8 kullaniyor, v7 1/1, momentum 5/8, v6 farkli. v7cd faz: 6/8.
        time.sleep(SCAN_INTERVAL_SEC * 6 / 8)
        while True:
            try:
                self.tick()
            except Exception:
                log.exception("v7cd tick hatasi")
            ...
            while True:
                ...
                try:
                    ...
                    log.exception("v7cd hizli cikis hatasi")
    ...
    def _belirsiz_takip(self) -> None:
        ...
        if durum == "gerceklesti" and detay and detay.get("fiyat", 0) > 0:
            ...
            log.warning("V7CD BELIRSIZ SONUC %s: tx zincirde yok, iptal", aday["pair"])
        else:
            log.critical("V7CD BELIRSIZ SONUC %s: cozulemedi (%s)", aday["pair"], durum)
    ...
    def _belirsiz_pozisyon_ac(self, aday: dict, detay: dict) -> None:
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        log.warning("V7CD BUY (mutabakat) %s $%.2f @ %.8g", aday["pair"], usd, entry)
    ...
    def _enter(self, client: httpx.Client) -> None:
        ...
        try:
            ...
            log.warning("V7CD giris tick atlandi, tarama hatasi: %r", e)
            return
        ...
        for pr in pairs:
            ...
            _izin, _red_nedeni = aday_paylastir.iddia_et(pr.token_address, "v7cd", pr.name)
            if not _izin:
                continue
        ...
        if sol_h1 is None:
            if not self._regime_logged:
                ...
                log.warning("V7CD REJIM: sol_h1 verisi yok (fail-closed), giris kapali")
            rejim_reject_kaydet(cands, "V7CD", None)
            return
        if sol_h1 < SOL_H1_MIN:
            if not self._regime_logged:
                self._regime_logged = True
                log.warning("V7CD REJIM: sol_chg_h1 %.2f%% < %.2f%%, giris yok",
                            sol_h1, SOL_H1_MIN)
            rejim_reject_kaydet(cands, "V7CD", sol_h1)
            return
        ...
        for pair in cands:
            ...
            try:
                ...
                safety_reject_kaydet(pair, "V7CD", "safety_hata", type(e).__name__)
                continue
            time.sleep(0.2 if self._aggressive else 1.5)
            if not report.ok:
                safety_reject_kaydet(
                    pair, "V7CD", report.kapi or "safety_red",
    ...
    def _open_position(self, pair, usd: float, sol_h1: float | None = None,
        ...
        if self.balance < usd + gas:
            return False
        taze = taze_teyit(pair, "V7CD", client)
        if taze.iptal:
            log.warning("V7CD GIRIS IPTAL %s: taze fiyat taramanin %%%.2f ustunde (kaynak %s)",
        ...
        if not devam:
            if self._son_exec_neden == "islem_belirsiz":
                ...
                log.critical("V7CD GIRIS BELIRSIZ %s: zincir mutabakati bekleniyor",
                             pair.name)
                return False
            log.error("V7CD GIRIS IPTAL %s: canli alim gerceklesmedi", pair.name)
            kritik_uyari("GIRIS IPTAL", f"giris:v7cd:{pair.name}", f"V7CD {pair.name}: canli alim gerceklesmedi (broker fail)")
            return False
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        aday_paylastir.kaydet(pair.token_address, "v7cd", pair.name)
        log.warning("V7CD BUY %s $%.2f @ %.8g (h1 %.1f%%, liq $%.0f, yas %s)",
                    pair.name, usd, eff_price, pair.chg_h1, pair.liquidity_usd, yas_str(pair.pool_created_at))
        notify("[V7CD] ALIM: %s $%.2f @ %.8g (h1 %%%.1f, liq $%.0f)"
    ...
    def _eval_position(self, pos: dict, price: float, now: float,
        ...
        price, ariza = guard_price(pos, price, now, "V7CD", liquidity_usd=liquidity_usd)
        if ariza:
            return None
        ...
        # runner_mode aktifse peak izle + trail cikis
        if pos.get("runner_mode"):
            peak = float(pos.get("runner_peak") or entry)
            if price > peak:
                pos["runner_peak"] = price
                return None
            # tepenin %TRAIL_PCT altina inince kalanı sat
            if price <= peak * (1 - TRAIL_PCT / 100.0):
                return "runner_trail"
            return None
        # ilk asama: TP+%2 hit -> kismi sat + kalani runner
        if pnl_pct > TP_PCT:
            return "tp_2_partial"
        return None
    ...
    def _fiyat_tazelendi(self, pos: dict, now: float) -> None:
        ...
        if pos.pop("kor_fiyat", None):
            ...
            log.warning("V7CD kor fiyat sona erdi %s", pos["pair"])
            ...
            if price is None or price <= 0:
                ...
                if taze_yas >= KOR_FIYAT_SEC:
                    ...
                    if now - pos.get("_kor_alarm_ts", 0.0) >= KOR_ALARM_ARALIK_SEC:
                        ...
                        log.critical(
                            "V7CD KOR FIYAT %s: %.0fs'dir taze fiyat yok "
    ...
    def _close_position(self, pos: dict, price: float, reason: str, now: float) -> None:
        ...
        for i in range(deneme):
            ...
            if devam:
                break
            if i + 1 < deneme:
                log.warning("V7CD SATIS TEKRAR %s (%s): deneme %d/%d basarisiz",
                            pos["pair"], reason, i + 1, deneme)
                time.sleep(STOP_RETRY_SEC)
        if not devam:
            pos["_sat_bekle_ts"] = time.time() + SAT_COOLDOWN_SEC
            log.error("V7CD SATIS ERTELENDI %s: canli satis gerceklesmedi", pos["pair"])
            kritik_uyari("SATIS ERTELENDI", f"sat:v7cd:{pos['pair']}", f"V7CD {pos['pair']}: canli satis fail, retry cooldown")
            return
        if canli is not None and canli.fiyat > 0:
            eff_price = canli.fiyat
        ...
        if kismi:
            ...
            self._save()
            log.warning("V7CD PARTIAL %s: yarisi satildi pnl $%.2f (%.2f%%), "
                        "runner devam (peak=%.8g, trail=%.1f%%)",
                        pos["pair"], pnl, pnl_pct, pos["runner_peak"], TRAIL_PCT)
            notify("[V7CD] PARTIAL: %s +$%.2f (%%%.2f) — kalani runner (peak %.8g)"
        ...
        log.warning("V7CD SELL %s pnl $%.2f (%.2f%%) — %s, hold %.0fs (mfe %.1f%% mae %.1f%%)",
                    pos["pair"], pnl, pnl_pct, reason, hold_sec, pos["mfe_pct"], pos["mae_pct"])
        notify("[V7CD] SATIM: %s pnl $%.2f (%%%.2f) — %s, hold %.0fdk"
