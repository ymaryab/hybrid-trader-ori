"""R1 Runner Catcher paper motoru — h1>50 aday + TP+%50 kismi + trail (18 Tem).
...
MOD: CANLI_MOTOR env dispatch'ine tabi. 19 Tem'den beri CANLI 10. motor
canli hatti devraldi (canli_session.py) — CANLI_MOTOR=canli ise R1 tekrar
PaperExec alir. CANLI_MOTOR=r1 ise R1 canli emir keser (tarihsel yol).
R1_ENABLED=0 ile kapatilir.

Diger motorlara SIFIR dokunus. Sadece su dosyalara yazar:
  data/r1_state.json   (sanal bakiye + acik pozisyonlar)
  data/r1_trades.jsonl (her sanal kapanista kayit)
"""
...
TP_PCT = float(os.getenv("R1_TP_PCT", "10.0"))
SOL_H1_MIN = float(os.getenv("R1_SOL_H1_MIN", "0.35"))
...
EXIT_SLIPPAGE_BPS = {"tp_partial_1": 300, "tp_partial_2": 300,
...
# R1 Runner Catcher ozel — koruma katmani
DISASTER_PCT = float(os.getenv("R1_DISASTER_PCT", "-15.0"))   # felaket freni
GRACE_SEC = int(os.getenv("R1_GRACE_SEC", str(15 * 60)))      # 15dk sabir
LATE_STOP_PCT = float(os.getenv("R1_LATE_STOP_PCT", "-5.0"))  # 15dk sonrasi -%5 alti stop
CEILING_SEC = int(os.getenv("R1_CEILING_SEC", str(120 * 60))) # 120dk tavan
M5_MIN = float(os.getenv("R1_M5_MIN", "0.0"))                 # m5 > 0 zorunlu (yorgun aday elemek)

# Trail-arm: TP kismi + runner trail
KISMI_ORAN = float(os.getenv("R1_KISMI_ORAN", "0.5"))    # deprecated (v2)
# 19 Tem 3-asama kismi cikis: 1/3 TP+10, 1/3 TP+17, 1/3 runner trail
# (24 Tem kullanici karari: 2. asama 25 -> 17)
...
class R1Engine:
    ...
    def __init__(self, settings) -> None:
        ...
        self._exec, self._exec_arizali = init_motor_exec("r1")
        self._load()
    ...
    def _restore_day_realized(self) -> None:
        ...
        try:
            ...
            log.debug("R1 gun ici pnl geri yuklenemedi", exc_info=True)
    ...
    def _entries_blocked(self) -> str | None:
        ...
        if kill_is_active():
            if not self._kill_logged:
                ...
                log.critical("R1: kill-switch AKTIF, yeni girisler durdu (cikislar suruyor)")
            return "kill_switch"
        if self._kill_logged:
            self._kill_logged = False
            log.warning("R1: kill-switch kalkti, girisler serbest")
        if DAILY_LOSS_LIMIT_USD > 0 or self._pct_limit_aktif():
            ...
            if limit is None and not kesin:
                if not self._limit_belirsiz_logged:
                    ...
                    log.critical("R1: gun limiti hesaplanamadi, yeni giris kapali (fail-closed)")
                return "daily_limit_belirsiz"
            if self._limit_belirsiz_logged:
                self._limit_belirsiz_logged = False
                log.warning("R1: gun limiti hesaplandi, belirsizlik kalkti")
            if limit is not None and self._day_realized <= -limit:
                if not self._limit_logged:
                    self._limit_logged = True
                    log.critical(
                        "R1: gunluk zarar limiti asildi ($%.2f <= -$%.2f), "
    ...
    def _canli_mtm(self) -> float | None:
        try:
            ...
            log.debug("R1 canli MTM okunamadi", exc_info=True)
        return None
    ...
    def _gun_limiti(self) -> tuple[float | None, bool]:
        ...
        self._save()
        log.warning("R1 gun limiti sabitlendi: MTM $%.2f x %%%g = $%.2f",
    ...
    def _exec_fill(self, yon: str, token_address: str, *, usd: float = 0.0,
        ...
        try:
            ...
            log.error("R1 yurutme hatasi (%s %s): %s", yon, token_address[:8], e)
            fill = None
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            fh.close()
            log.critical("R1: baska bir instance calisiyor, motor baslatilmiyor")
            return False
    ...
    def run_forever(self) -> None:
        if not self._acquire_lock():
            return
        log.warning(
            "R1 paper basladi (TP=+%%%.1f tek cikis, stop yok) - sanal $%.2f · "
    ...
    def _belirsiz_takip(self) -> None:
        ...
        if durum == "gerceklesti" and detay and detay.get("fiyat", 0) > 0:
            ...
            log.warning("R1 BELIRSIZ SONUC %s: tx zincirde yok, iptal", aday["pair"])
        else:
            log.critical("R1 BELIRSIZ SONUC %s: cozulemedi (%s)", aday["pair"], durum)
    ...
    def _belirsiz_pozisyon_ac(self, aday: dict, detay: dict) -> None:
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        log.warning("R1 BUY (mutabakat) %s $%.2f @ %.8g", aday["pair"], usd, entry)
    ...
    def _enter(self, client: httpx.Client) -> None:
        ...
        try:
            ...
            log.warning("R1 giris tick atlandi, tarama hatasi: %r", e)
            return
        ...
        for pr in pairs:
            ...
            _izin, _red_nedeni = aday_paylastir.iddia_et(pr.token_address, "r1", pr.name)
            if not _izin:
                continue
        ...
        for pair in cands:
            ...
            try:
                ...
                safety_reject_kaydet(pair, "R1", "safety_hata", type(e).__name__)
                continue
            time.sleep(0.2 if self._aggressive else 1.5)
            if not report.ok:
                safety_reject_kaydet(
                    pair, "R1", report.kapi or "safety_red",
    ...
    def _open_position(self, pair, usd: float, sol_h1: float | None = None,
        ...
        if self.balance < usd + gas:
            return False
        taze = taze_teyit(pair, "R1", client)
        if taze.iptal:
            log.warning("R1 GIRIS IPTAL %s: taze fiyat taramanin %%%.2f ustunde (kaynak %s)",
        ...
        if not devam:
            ...
            "pool_yas_dk": (round((time.time() - float(pair.pool_created_at)) / 60.0, 1)
                ...
                log.critical("R1 GIRIS BELIRSIZ %s: zincir mutabakati bekleniyor",
                             pair.name)
                return False
            log.error("R1 GIRIS IPTAL %s: canli alim gerceklesmedi", pair.name)
            return False
        ...
        if feed is not None:
            feed.add_pool(pos["pool_address"])
        aday_paylastir.kaydet(pair.token_address, "r1", pair.name)
        log.warning("R1 BUY %s $%.2f @ %.8g (h1 %.1f%%, liq $%.0f, yas %s)",
    ...
    def _eval_position(self, pos: dict, price: float, now: float,
        ...
        price, ariza = guard_price(pos, price, now, "R1", liquidity_usd=liquidity_usd)
        if ariza:
            return None
        ...
        if asama < 1 and pnl_pct >= TP_PCT:
            return "tp_partial_1"
        # 4) 15dk grace sonrasi -%5 alti stop
    ...
    def _fiyat_tazelendi(self, pos: dict, now: float) -> None:
        ...
        if pos.pop("kor_fiyat", None):
            ...
            log.warning("R1 kor fiyat sona erdi %s", pos["pair"])
            ...
            if price is None or price <= 0:
                ...
                if taze_yas >= KOR_FIYAT_SEC:
                    ...
                    if now - pos.get("_kor_alarm_ts", 0.0) >= KOR_ALARM_ARALIK_SEC:
                        ...
                        log.critical(
                            "R1 KOR FIYAT %s: %.0fs'dir taze fiyat yok "
    ...
    def _close_position(self, pos: dict, price: float, reason: str, now: float) -> None:
        if time.time() < pos.get("_sat_bekle_ts", 0.0):
            return
        # 19 Tem 3-asama kismi cikis:
        #   tp_partial_1: orjinalin 1/3 sat, kalan 2/3 tut
        #   tp_partial_2: kalanin yarisi (=orjinal 1/3) sat, kalan 1/3 runner
        #   runner_trail: kalan tumu sat (~1/3)
        ...
        if reason == "tp_partial_1":
            ...
            satilan_oran = KISMI_ORAN  # geri uyum (eski kayitlar)
        else:
            satilan_oran = 1.0
        ...
        for i in range(deneme):
            ...
            if devam:
                break
            if i + 1 < deneme:
                log.warning("R1 SATIS TEKRAR %s (%s): deneme %d/%d basarisiz",
                            pos["pair"], reason, i + 1, deneme)
                time.sleep(STOP_RETRY_SEC)
        if not devam:
            pos["_sat_bekle_ts"] = time.time() + SAT_COOLDOWN_SEC
            log.error("R1 SATIS ERTELENDI %s: canli satis gerceklesmedi", pos["pair"])
            return
        if canli is not None and canli.fiyat > 0:
            eff_price = canli.fiyat
        ...
        if kismi:
            ...
            if reason == "tp_partial_1":
                pos["kismi_asama"] = 1
                # 1. asamada runner_mode HENUZ acilmaz — 2. asama %25'e ulasmasini bekle
                self._save()
                log.warning("R1 PARTIAL-1/3 %s: TP+%%%.0f, 1/3 satildi pnl $%.2f (%.2f%%), "
                            "%.0f%% i +%.0f%% bekliyor, kalan 1/3 runner",
                            pos["pair"], TP_PCT, pnl, pnl_pct, 100/3, TP2_PCT)
                notify("[R1] PARTIAL-1/3: %s +$%.2f (%%%.2f) — 1/3 sirada +%%%.0f"
                ...
                self._save()
                log.warning("R1 PARTIAL-2/3 %s: 1/3 daha satildi pnl $%.2f (%.2f%%), "
                            "kalan 1/3 runner (peak=%.8g, trail=%.1f%%)",
                            pos["pair"], pnl, pnl_pct, pos["runner_peak"], TRAIL_PCT)
                notify("[R1] PARTIAL-2/3: %s +$%.2f (%%%.2f) — kalan 1/3 runner (peak %.8g)"
        ...
        log.warning("R1 SELL %s pnl $%.2f (%.2f%%) — %s, hold %.0fs (mfe %.1f%% mae %.1f%%)",
