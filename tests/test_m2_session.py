...
def test_fast_exit_tick_closes_tp_with_fast_source(m2_data_dir, monkeypatch):
    eng = M2Engine(_settings())
    pos = _open(eng)
    sample_ts = time.time() - 0.4
    feed = _StubFeed({pos["pool_address"]: (pos["entry_price"] * 1.013, sample_ts)})
    monkeypatch.setattr(m2, "get_feed", lambda: feed)
    eng.fast_exit_tick()
    t = _last(m2_data_dir)
    assert t["exit_reason"] == "tp_1_2"
...
def test_fast_exit_tick_skips_position_without_fresh_price(m2_data_dir, monkeypatch):
    eng = M2Engine(_settings())
    pos = _open(eng)
    monkeypatch.setattr(m2, "get_feed", lambda: _StubFeed({}))
    eng.fast_exit_tick()
    assert eng.positions == [pos]  # taze fiyat yoksa dokunmaz, 30s tick kapsar
    assert not (m2_data_dir / m2.TRADES_FILE).exists()


def test_fast_exit_tick_noop_when_feed_disabled(m2_data_dir, monkeypatch):
    eng = M2Engine(_settings())
    pos = _open(eng)
...
def test_manage_exits_poll_path_records_source_and_null_delay(m2_data_dir, monkeypatch):
    eng = M2Engine(_settings())
    pos = _open(eng)
    _tick_price(eng, pos, pos["entry_price"] * 1.013, pos["opened_ts"] + 60, monkeypatch)
    t = _last(m2_data_dir)
...
def test_manage_exits_prefers_fast_feed_over_poll(m2_data_dir, monkeypatch):
    eng = M2Engine(_settings())
    pos = _open(eng)
    sample_ts = time.time()
    feed = _StubFeed({pos["pool_address"]: (pos["entry_price"] * 1.013, sample_ts)})
    ...
    monkeypatch.setattr(m2, "fetch_pool_price", _boom)
    eng._manage_exits(client=SimpleNamespace())
    t = _last(m2_data_dir)
    assert t["exit_reason"] == "tp_1_2"
    assert t["price_source"] == "fast"
